...
export default function StatusOverview({ rows, onOpenDetail }) {
  ...
  const persist = (key, obj) => { try { localStorage.setItem(key, JSON.stringify(obj)); } catch {} };
  ...
  const card = {
    ...
    input: {
      padding: '8px 10px', border: '1px solid #e5e7eb', borderRadius: 8, minWidth: 220,
      backgroundColor: '#fff', color: '#111', fontSize: 14, outline: 'none',
    },
    select: {
    ...
    row: {
      ...
      gap: 12, alignItems: 'center', padding: 12, background: '#fff',
  ...
  const pillFor = (status, manual) => {
    if (manual) return card.pill('#e5e7eb', '#374151');
    switch (status) {
  ...
  return (
    <div>
      <div style={card.controls}>
        <input placeholder="Suchen…" value={search} onChange={(e) => setSearch(e.target.value)} style={card.input} />
      ...
      <div style={{ display: 'grid', gap: 12 }}>
        ...
        {filtered.map((s) => (
          <div key={s.key} style={card.row}>
            <div><span style={{ fontWeight: 700, color: '#111' }}>{s.kunde_name}</span></div>
            <div style={card.meta}>{s.kundenrolle || '—'}</div>
            <div>
            ...
            <div style={card.meta}>{s.last ? new Date(s.last).toLocaleString('de-DE') : '—'}</div>
